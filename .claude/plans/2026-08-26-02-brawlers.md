...
**능력 상세 정보는 이 플랜에 포함됐다.** 스타파워·가젯·기어의 현지화된 이름(424/424, 671/671)과 설명(424/424)을 붙였다. 수치는 게임 CSV 참조를 따라가 채우고(411개), 경로가 없어 못 푸는 자리만 "일정 비율" 같은 자연어로 대체한다. 기어는 설명이 없어 수치(`+15%`)를 대신 보여준다. 소스와 한계는 API 레퍼런스 2-5 에 정리돼 있다.
