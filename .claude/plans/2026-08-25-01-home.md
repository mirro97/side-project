...
**상위 문서:** [설계 2-2](../specs/2026-08-25-brawl-stars-webapp-design.md) · [디자인 시스템](../specs/2026-08-25-design-system.md)
...
import { fetchPlayerRanking, fetchClubRanking, fetchEventRotation } from './api'
...
export function fetchPlayerRanking(country: string, limit: number, after?: string) {
  const cursor = after ? `&after=${after}` : ''
  return bsFetch<Paged<RankingEntry>>(`/rankings/${country}/players?limit=${limit}${cursor}`)
}

export function fetchClubRanking(country: string, limit: number, after?: string) {
  const cursor = after ? `&after=${after}` : ''
  return bsFetch<Paged<ClubRankingEntry>>(`/rankings/${country}/clubs?limit=${limit}${cursor}`)
}

export function fetchEventRotation() {
  return bsFetch<EventSlot[]>('/events/rotation')
}

export function fetchPlayer(tag: string) {
  return bsFetch<Player>(`/players/${encodeTag(tag)}`)
}

export function fetchBattlelog(tag: string) {
...
import { fetchClubRanking, fetchEventRotation, fetchPlayerRanking } from '@/lib/bs/api'
...
export async function GET() {
  const [players, clubs, events] = await Promise.allSettled([
    fetchPlayerRanking('global', 5),
    fetchClubRanking('global', 5),
    fetchEventRotation(),
  ])
...
import { NextResponse } from 'next/server'
import { fetchPlayer } from '@/lib/bs/api'
...
export async function GET(_req: Request, { params }: { params: Promise<{ tag: string }> }) {
  ...
  try {
    return NextResponse.json({ ok: true, data: await fetchPlayer(decoded) })
  } catch (e) {
...
- **`fetchPlayerRanking` / `fetchClubRanking`** — 플랜 03 이 커서 파라미터를 써서 재사용한다
- **`fetchEventRotation` / `sortByEndingSoon`** — 플랜 04 이벤트 탭이 그대로 쓴다
