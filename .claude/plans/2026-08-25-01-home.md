...
```
  공유 (모든 방문자에게 동일)     유저 순위 · 클랜 순위 · 이벤트
    → 서버 컴포넌트에서 병렬 호출하고 revalidate 로 10분 캐시
    → 직접 SSR 하므로 별도 라우트도, 클라이언트 왕복도 없다
...
**세 개를 순차로 부르면 1.5초가 된다.** 페이지에서 `Promise.allSettled` 로 병렬 호출한다.
...
### Task 3: (삭제됨) 공유 데이터 라우트

**만들지 않는다.** 설계 결정대로 서버 컴포넌트가 공유 데이터를 직접 가져오므로 `/api/home` 라우트는 아무도 부르지 않는다. Task 8 이 페이지에서 `Promise.allSettled` 로 병렬 호출하고 `revalidate` 로 캐시한다.

클라이언트가 공유 데이터를 다시 불러야 하는 요구가 생기면 그때 만든다.
...
플레이어와 클럽이 같은 컴포넌트를 쓴다. **플랜 03 랭킹 탭이 그대로 가져다 쓴다.**

`kind` 같은 변형 prop 을 두지 않는다. 실제 렌더 차이가 없어서 쓰이지 않는 prop 이 되고, 미사용 코드 금지 규칙에 걸린다. 차이는 넘기는 값(`nameColor`, `subtitle`)으로 이미 표현된다.
...
describe('RankRow', () => {
  it('플레이어 변형은 이름과 클럽을 보여준다', () => {
    wrap(<RankRow rank={1} name="VITAL SHARK" trophies={317958}
  ...
  it('nameColor 가 있으면 이름에 적용한다', () => {
    wrap(<RankRow rank={1} name="C" trophies={1} iconUrl="x.png"
                  nameColor="0xffcb5aff" />)
    expect(screen.getByText('C')).toHaveStyle({ color: '#cb5aff' })
  })

  it('클럽 변형은 멤버 수를 보여준다', () => {
    wrap(<RankRow rank={1} name="Heaven" trophies={7484545}
...
그리고 **레이아웃과 페이지 양쪽에서 `setRequestLocale(locale)` 을 먼저 부른다.** next-intl 이 헤더를 읽으면 라우트가 Dynamic 으로 빠져 `revalidate` 가 무시된다. 빌드 출력에서 `●` 가 아니라 `ƒ` 로 나오면 이 상태다.
...
- [ ] 프로덕션 빌드에서 홈 응답이 10ms 미만이다 (SSG 확인). dev 서버 수치는 컴파일 시간이 섞여 의미가 없다
- [ ] 게임모드 아이콘이 깨지지 않는다 (`imageId` 사용 확인)
- [ ] 이름에 `<c3>` 같은 색상 마크업이 글자로 보이지 않는다
- [ ] 빌드 결과에서 `/en`·`/ko` 가 `●` (SSG) 다. `ƒ` (Dynamic) 이면 캐시가 먹지 않는다
