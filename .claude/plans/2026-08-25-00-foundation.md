 ...
 * TID 를 추측하지 말 것 — 로케일에서 한글명을 역으로 찾아 확인한 값이다.
 */
 ...
 * 0 으로 들어오는 브롤러가 있어(BOLT) nullish 병합만으로는 부족하다.
 * 0 이 통과하면 정규화 후 "최근접"으로 잘못 취급되고 폴백도 걸리지 않는다.
 */
...
경고 20건
  역할 없음: WENDY
  ... (BrawlAPI class 가 Unknown 인 19종)
  사거리 없음: BOLT
```
