...
**Architecture:** Next.js App Router에서 Route Handler가 브롤스타즈 API 프록시 역할을 하고, 브롤러·게임모드 같은 정적 데이터는 빌드타임에 JSON으로 생성해 번들에 넣는다. 개인 설정은 로컬스토리지에 둔다. **v1은 서버 상태가 없다** — DB는 v1.5다.

**Tech Stack:** Next.js App Router · TypeScript · Tailwind v4 · shadcn CLI v4 · next-intl · Wanted Sans · Vitest
...
```
  00  공통 기반   (이 문서)   타입 · 토큰 · 파이프라인 · API 계층 · 공통 컴포넌트
  01  홈                      /api/home · 4개 섹션
  02  브롤러                   그리드 · 검색 · 상세 패널
...
**Files:**
- Create: `src/i18n/routing.ts`, `src/i18n/request.ts`, `src/proxy.ts`, `messages/en.json`, `messages/ko.json`, `src/app/[locale]/layout.tsx`, `src/app/[locale]/page.tsx`
...
```ts
// src/proxy.ts
// Next.js 16 에서 middleware 규약이 proxy 로 바뀌었다.
// 파일명만 다르고 next-intl 은 그대로 동작한다.
...
### Task 16: DB 스키마 — v1.5로 이동

**이 태스크는 v1에서 수행하지 않는다.**

DB가 필요한 기능(트로피 추이 그래프, 25건 초과 전투 통계, AI 실시간 호출 레이트리밋)이 전부 v1.5로 밀렸다. 여섯 페이지 중 다섯이 DB 없이 완전히 동작하고, 추이 그래프는 데이터가 며칠 쌓여야 의미가 생기므로 배포 직전에 붙여도 잃는 것이 없다.

스키마 설계는 상위 설계 문서 4-5에 남아 있다. v1.5 플랜에서 그대로 가져다 쓴다.
