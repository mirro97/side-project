...
`mockup/`과 `.claude/`는 유지된다. `.gitignore`가 덮어써지면 `.env.local` 항목이 남아 있는지 확인한다.
...
```bash
find node_modules/wanted-sans -name "*Variable*.woff2" | head
```

아래 Step 3의 `src` 경로가 이 출력과 다르면 출력값으로 바꾼다.

- [ ] **Step 3: next/font/local 설정**
...
export const wantedSans = localFont({
  src: '../../node_modules/wanted-sans/fonts/webfonts/variable/complete/WantedSansVariable.woff2',
  weight: '100 900',
  display: 'swap',
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" className={wantedSans.variable}>
      <body>{children}</body>
    </html>
  )
}
