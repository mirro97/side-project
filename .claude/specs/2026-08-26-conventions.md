...
## 5-4. 생성물은 필요한 만큼만 다시 만든다

`npm run build:data` 와 `npm run build:ai` 의 산출물은 **레포에 커밋된다.** 전면 재생성은
git 히스토리에 그만큼의 블롭을 영구히 남기므로 습관적으로 돌리지 않는다.

```
  전면 재생성    게임 업데이트로 브롤러·모드가 추가됐을 때
  --force       프롬프트나 생성 규칙이 바뀌어 전체가 흔들려야 할 때
  --id <id>     한 종의 문구만 다듬을 때
```

**AI 생성물은 사람이 고친 것을 보존한다.** `--force` 없이는 기존 파일을 덮지 않고,
색인은 이번 실행 결과가 아니라 **디스크의 파일에서** 다시 만든다. 손으로 고친 문장이
다음 실행에 지워지지 않는 게 DB 대신 파일을 고른 이유다.

크기는 문제가 아니다 — 상세용 파일은 108종 전부 합쳐 130KB 이고 한 번에 한 장(gzip 0.4KB)만
받는다. 번들에 들어가는 건 성향 색인뿐이고 gzip 6.9KB 다.
